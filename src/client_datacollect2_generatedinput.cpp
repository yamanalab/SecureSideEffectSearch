...
//
// Usage: client_datacollect
//
// Generator of a local set of inputs for test
    ...
    */
    ...
    for (int t = 0; t < 200; ++t)
    {
        ...
        */
        ...
        if (!connect){
          cerr<<return_current_time_and_date()<<" Can not connect:
        "<<connect.error().message()<<endl; return 1;
        }
        ...
        assert(numSide>0);
        //cout<<"Enter side effect ID (total "<<numSide<<", 0-"<<totSide-1<<"):
        "; vector<int> sides; for (int i=0;i<numSide;++i){ int sideID;
        ...
        fout<<percentage<<"\t"<<numRes<<"\t"<<diff.count()<<"\t"<<exec_time<<"\t"<<comm_time<<"\t"<<diff_dec.count()<<endl;
